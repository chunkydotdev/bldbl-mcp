...
exports.BuildableMCPClient = void 0;
exports.createBuildableClient = createBuildableClient;
const axios_1 = __importDefault(require("axios"));
const uuid_1 = require("uuid");
class BuildableMCPClient {
    constructor(config, options = {}) {
        ...
        this.aiAssistantId =
            config.aiAssistantId || `ai_${(0, uuid_1.v4)().substring(0, 8)}`;
        ...
        this.log('info', `Buildable MCP Client initialized for project ${config.projectId}`);
    }
    /**
    ...
    async getProjectContext() {
        ...
        try {
            const response = await this.makeRequest('GET', `/projects/${this.config.projectId}/context`);
    ...
    async getNextTask() {
        ...
        try {
            const response = await this.makeRequest('GET', `/projects/${this.config.projectId}/next-task`);
    ...
    async startTask(taskId, options = {}) {
        ...
        try {
            const response = await this.makeRequest('POST', `/tasks/${taskId}/start`, {
    ...
    async updateProgress(taskId, progress) {
        this.log('debug', `Updating progress for task ${taskId}: ${progress.progress}%`);
        try {
            const response = await this.makeRequest('POST', `/tasks/${taskId}/progress`, {
    ...
    async createDiscussion(discussion) {
        ...
        try {
            const response = await this.makeRequest('POST', `/projects/${this.config.projectId}/discuss`, {
                type: 'question',
    ...
    async getConnectionStatus() {
        try {
            const response = await this.axios.get(`/projects/${this.config.projectId}/ai-connections`);
            const connections = (response.data.connections || []);
            const myConnection = connections.find((conn) => conn.ai_assistant_id === this.aiAssistantId);
            return (myConnection || {
                status: 'disconnected',
                connected_at: '',
                last_activity_at: '',
    ...
    async updateConnectionStatus(status, currentTaskId) {
        try {
            ...
            this.log('debug', 'Connection status update failed (non-critical):', error);
        }
    }
    formatError(error) {
        const apiError = {
...
exports.BuildableMCPClient = BuildableMCPClient;
// Export default instance creator
function createBuildableClient(config, options) {
    return new BuildableMCPClient(config, options);
}
// Export for convenience
exports.default = BuildableMCPClient;
